// irrWeatherEx.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
int main()
{
	...
	ICloudLayer* layer = weatherManager->addCloudLayer(vector3df(0, 19000, 0), info, 50, false);
	...
	while (device->run())
	{
		...
		IWeatherManagerAtmosphere* pManager = weatherManager->getAtmosphere();

		swprintf(tmp, 1024, L" [irrWeatherManager Sample] FPS: %d Dayspeed: %5.3f",
			fp, pManager->getDayspeed());
